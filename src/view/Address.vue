<template>
  <div>
              ...
              <ul>
                <li v-for="item in AddressList">
                  ...
                  <div class="addr-opration addr-del">
                    <a href="javascript:;" class="addr-del-btn" @click="checkDelete(item.addressId)">
                  ...
                  <div class="addr-opration" :class="{'addr-default':item.isDefault}" v-show="item.isDefault">默认地址</div>
                </li>
                <li class="addr-new" @click="addressModelFlag=true">
    ...
    <modal :mdShow="modalConfirm">
      ...
      <!-- 添加地址 -->
    <div class="md-modal modal-msg md-modal-transition" :class="{'md-show':addressModelFlag}" >
      <div class="md-modal-inner">
        <div class="md-top">
          <div class="md-title">添加用户地址</div>
          <button class="md-close" @click="addressModelFlag = false">关闭</button>
        </div>
          ...
          <div class="confirm-tips">
            ...
            <ul>
              <li class="regi_form_input">
                ...
                <input type="text" tabindex="1" name="loginname" placeholder="Name" v-model="userName" data-type="loginname" class="regi_login_input regi_login_input_left">
              </li>
              ...
              <li class="regi_form_input noMargin">
                ...
                <input type="postcode" tabindex="3" name="postcode" placeholder="postcode" v-model="postcode" class="regi_login_input regi_login_input_left login-input-no input_text" @keyup.enter="login">
              </li>
            </ul>
          </div>
          <div class="login-wrap">
            <button class="btn-login" @click="register" v-bind:disabled="checkReg">确 认</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    getAddress() {
      ...
      axios.get('users/address',{params:param}).then( (response) => {
        let res = response.data;
        // console.log(response)
        if (res.status === '0') {
          this.AddressList = res.result[0].addressList
        }
      })
    },
    checkDelete(addressId) {
